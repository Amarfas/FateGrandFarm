...
class PrintText():
    ...
    def data_prefix( self, line_break = False ):
        if line_break:
            path1 = os.path.join( PrintText().debug_path()    , 'Data Files Test' )
            path2 = os.path.join( PrintText().debug_path(True), 'Data Files Test 2' )
    ...
    if tests['Line Break']:
        ...
        for mode in [ 1, 2, 3 ]:
            if mode in tests['Modes']:
                txt = 'Line Break Test probably cannot handle Test #' + str(mode)
                PrintText().print(txt, True)
...
def check_matrix( text, norm, test, np_array = True, extra = False, extra_test = False ):
    valid_2 = 'NA'
    if np_array:
        try:
