...
# Mode 1-6: Compare variable values: 
#    1 = 'Node Names', 'AP Costs', and 'Drop Matrices';
#    2 = 'ID to Index', 'Skip Data Index', and 'Index to Name';
#    3 = 'Run Matrix' and 'Run Caps'; 4 = 'Planner'; 5 = 'Debug'; 6 = 'Output'
# Mode 7: Compare times in 'Build Matrix' and 'Planner'.
...
# 'Line Break' will have one side using APD and Calc csv's with line breaks
...
# 'Test' has everything but Octuplets, including XP.
#       Notably 1000+ Moonlight to break Run Caps
...
# Input different config changes you would like to automatically be tested
change_config = {
    'Event Cap':                [2000, 0, 1000] ,
...
class Toolkit():
    ...
    def test_planner(self, new_code = True, mes = 0):
        if new_code:
            planner = plan.Planner(self.nodes, self.input, self.run_mat, mes)
...
def change_time( test_package, test, timer, t1, t2 ):
    ...
    ex.PrintText().print(f' {test} Difference x' + format(mult,',')
                         + f': {str(time_dif)}\n')
    
    goals = test_package['Goals']
    config = test_package['Config']
    if timer['No Extreme'] and time_dif > 10000:
        timer['Skip'].append({'Time': time_dif, 'Goals': goals,
                              'Config': config})
        return timer
...
def test_1(nodes: QuestData, nodes2: QuestData):
    names = [nodes.quest_names, nodes2.quest_names]
    ap_costs = [nodes.AP_costs, nodes2.AP_costs]
    matrix = [nodes.drop_matrix, nodes2.drop_matrix]

    ex.check_matrix('Nodes Names', names, False)
    ex.check_matrix('AP Cost', ap_costs)
    ex.check_matrix('Drop Matrix', matrix)
...
def test_2(goals, pre):
    ...
    id_to_index = [i1.ID_to_index, i2.ID_to_index]
    skip_data_index = [i1.skip_data_index, i2.skip_data_index]
    index_to_name = [i1.index_to_name, i2.index_to_name]

    eq1 = (id_to_index[0] == id_to_index[1])
    eq2 = (skip_data_index[0] == skip_data_index[1])
    eq3 = (index_to_name[0] == index_to_name[1])
    ex.PrintText().print('{:<{}}{}'.format('ID to Index equal:', 24, str(eq1)))
    ex.PrintText().print('{:<{}}{}'.format('Skip Data Index equal:', 24, str(eq2)))
    ex.PrintText().print('{:<{}}{}'.format('Index to Name equal:', 24, str(eq3)))
...
def test_3(run_caps: inter.RunCaps, run_caps2: inter.RunCaps):
    ...
    matrix = [run_cap_matrix['Matrix'], run_cap_matrix2['Matrix']]
    cap = [run_cap_matrix['List'], run_cap_matrix2['List']]

    ex.check_matrix('Run Matrix', matrix)
    ex.check_matrix('Run Cap', cap)
    ex.PrintText().check_valid()
...
def test_4(tool):
    ...
    runs = [sol.run_int, runs2]
    prob = [sol, prob2]
    total_ap = [sol.total_AP, tot_AP2]

    ex.check_matrix('Run Counts', runs, True, prob)
    if sol.total_AP == tot_AP2: 
        ex.PrintText().print("{:<{}}{}".format(
            'Total AP equal:', 24, 
            'T: AP = ' + str(sol.total_AP)
            ))
    else:
        txt = 'F: norm: '+ str(sol.total_AP) + ' != test: ' + str(tot_AP2)
        ex.PrintText().print('{:<{}}{}'.format('Total AP equal:', 24, txt))
...
def test_5():
    ...
    error = [[''.join(d1.error[0]), ''.join(d1.error[1])],
             d2.error]
    config = [d1.config_notes, d2.config_notes]
    monthly = [d1.monthly_notes, d2.monthly_notes]
    event = [d1.event_notes, d2.event_notes]
    lotto = [d1.lotto_notes, d2.lotto_notes]
    cap = [d1.run_cap_debug, d2.run_cap_debug]

    ex.check_matrix('Error', error)
    ex.check_matrix('Config Notes', config)
    eq = (monthly[0] == monthly[1])
    ex.check_matrix('Event Notes', event)
    ex.check_matrix('Lotto Notes', lotto)
    ex.check_matrix('Run Cap Debug', cap)
...
def test_6(tool):
    ...
    #txt = [[]]
    #for note in Plan.Output.text_written:
    #    txt[0].append([note[0], ''.join(note[1])])
    txt = [plan.Output.text_written]
    ...
    txt.append(PL.Output.text_written)

    ex.check_matrix('Output Files', txt, False)
    ex.PrintText().check_valid()

def test_building_drop(test_num, config, pre, tool: Toolkit, new_code):
    folder = 'Events Farm'
    if config['Folder'] != '':
        folder = os.path.join('Code', '_debug', 'Events_List',
                              config['Folder'])
...
def test_time_types(test_num, config, pre, goals, tool: Toolkit, new_code):
    if test_num == 7:
        ...
        file_path = glob.glob(os.path.join(pre, '*Calc.csv'))[0]
        if new_code:
            inter.DataFiles(goals, file_path)
        else:
            IN.DataFiles(goals, file_path)

    elif test_num >= 9 and test_num <= 11:
        test_building_drop(test_num, config, pre, tool, new_code)
    
    elif test_num == 12:
        tool.run_caps.build_run_cap_matrix()
        ...
        if new_code:
            output = plan.Output(tool.nodes)
            output.print_out(tool.solution)
        else:
...
def reset_debug(print):
    ...
    inter.Debug.error = [[], []]
    inter.Debug.quest_notes = {'Event': [], 'Lotto': [], 'Caps': []}
...
def config_loop(test_package, tests, timer):
    ...
    for goals in tests['Goals']:
        ...
        if ex.PrintText().check_failure(
                IN.ConfigList.settings['Run Count Integer'],
                tool['M'], goals):
            break

        tool['d'] = Toolkit(goals, pre[1], test_package['Config']['Folder'],
                            False)
...
inter.ConfigList().read_config_ini()
inter.Debug().note_config('The Path Prefix is ',
                          inter.path_prefix)
IN.standardize_path()
plan.Output.not_test = False
PL.Output.is_test = True
inter.Debug.notifications = tests['Print']
IN.Debug.notifications = tests['Print']

ex.PrintText().main_settings(tests, inter.path_prefix,
                             IN.path_prefix)
config_list, test_package, tests = ex.prepare_test_package(change_config,
                                                           tests)
...
config_main = ex.initial_config(test_package, tests)
