...
'Farming Plan Drops.txt' is very similar to 'Farming Plan.txt'. However, it also lists the expected number of each Material dropped for those runs. Note that if the 'Remove Zeros' configuration is on, it will not include undesired Materials (those with no goal).

'Config Notes' contains all the information necessary to properly understand the context for the corresponding 'Farming Plans'. This includes all the Errors that came up, Configurations, the Events included, Lotto Drop Rate boosts, the Monthly Ticket date range, and the applied Run Caps.
...
For the Monthly Exchange Ticket analysis, the 'Monthly Ticket Per Day' configuration caps at 4. The 'Monthly Ticket End Date' can either by a specific date or a time lapse relative to the starting date. If the starting date was 10/23/24 and '2 months' was entered, the end date will be 12/23/24. The program will accept relative time frames in 'days', 'months', or 'years'. Does not have to be plural ('day' will also be accepted).
...
To improve the 'Farming Plan', it is suggested to include a '.csv' for each Material that can be gotten from Lotto tickets in the 'Events Farm' folder. Extra data won't confuse the analysis and will instead help you in choosing Materials from those tickets.
 ...
 * Each column with a Material ID (used to prevent translation changes from affecting the program, hidden by default).
 * Each column with a Material Drop Rate.

Efficiency values in the sheet are not relevant for this program, so any settings in the 'Mat'/'Inputs tab do not affect this analysis.
...
## Upkeep
Similar to how the 'Event Quest' tab from the ['FGO Efficiency'](https://docs.google.com/spreadsheets/d/1CDQYB2Oa3YT1gfD6eT3hqRR7sVshQIQMKB_BOqDzTRU/) google sheet was used to produce the relevant Event '.csv's, the 'APD' and 'Calc' tabs were used to produce the relevant '.csv' files in the 'Data Files' Folder. If you want to update Fate/Grand Farm with new information, you need merely to download the relevant files and place them in the proper folders. For a more extensive explanation, read the next section ('Making Your Own Readable CSVs').

Adding new 'Monthly Ticket' CSVs takes slightly more work. Before downloading the data from the 'Monthly Ticket' tab as a '.csv' file, you have to input the corresponding month in cell B1 and year in cell D1, both as integers. Then the Material IDs corresponding to the optional Materials must be input into column C. It's recommended that you instead input the Material's name into the next cell in column D, and the sheet will automatically find and input the Material's ID.
 ...
 * FGF looks for the string 'Buyback?:' and considers the Lotto Event to have it if the next column is not white space/blank.
...
## 'Monthly Ticket' Files
 ...
 2. On the first row, input the month in the 2nd column and the year in the 4th column, as integers.
 3. Below the 1st row, input the value/string 'ID' into any cell.
 4. Input the Material IDs that correspond to the Monthly Exchange Ticket choices below 'ID', in the same column. These should match the IDs used in 'Calc'.
